...
data = urllib.parse.urlencode({"act": "login", "email": "ahlc@sina.cn", "password": "123456"}).encode(encoding='UTF8')
...
# 使用代理，以防止IP被封或IP次数受限：
proxy_handler = urllib.request.ProxyHandler(proxies={"http": "101.132.236.137:80"})
...
# (2)构建cookie,添加到cookiejar中 很多参数必传 先隐掉
# cookie = http.cookiejar.Cookie(name="xx", value="xx", domain="http://www.baidu.com")
...
response = urllib.request.urlopen("https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=2958864705,3762948589&fm=26&gp=0.jpg", timeout=120)
