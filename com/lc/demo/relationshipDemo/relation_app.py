            ...
            while s != selector:
                ...
                for item in _filter:
                    ...
                    if '#' in selector:
                        ...
                        break
            if status:
                if re.search(r',[w0],w|,[h1],h', selector):  # 同志关系去除
                    return []
        ...
        for i in ids:
            if options['type'] == 'chain':
                ...
                if items:
                    ...
                    if items:
                        result.extend(items)
    return list(set(result))  # unique(result) 使用set进行数组去重
...
print("relationship", '是:', '、'.join(g))

# 输出demo:
# 计算类型：1.从关系算称呼 2.从称呼算关系：
# 1
# 称呼方式：1.我称呼对方 2.对方称呼我：
# 1
# 我的性别：1.男 2.女
# 1
# 请输入待计算关系：
