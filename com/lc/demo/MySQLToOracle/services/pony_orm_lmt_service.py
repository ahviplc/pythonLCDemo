...
# 从models导出类
from models.models_for_lmt import MeterReportMonth, ScadaReportXNMid, ScadaReportXNWeek, ScadaReportXNMonth
...
def datas_from_mid_to_week_wash_data_oracle(org_id, srxm_list_that_week_min, srxm_qr, that_week_min, that_week_max):
    ...
    # 继续清洗数据的逻辑
    # 对 srxm_list_that_week_min 进行循环遍历 拿出其 SCADA_REPORT_XN_MID 的 表计编号
...
# datas_from_mid_to_month_wash_data_oracle
# 月数据清洗(月1号到月最后一天)版
# 数据从表SCADA_REPORT_XN_MID清洗到表SCADA_REPORT_XN_MONTH
# from ScadaReportXNMid to ScadaReportXNMonth
# org_id 机构号
# srxm_list_that_month_min 从Oracle查询出 【SCADA_REPORT_XN_MID】表 对应 月初mid数据 【下面srxm_list_that_month_max是月末mid数据数据 通过org_id, flmeter_no, that_week_max查询出】
# srxm_qr 是 <pony.orm.core.Query object>类型
# that_month_min 月初日期
# that_month_max 月末日期
@db_session
def datas_from_mid_to_month_wash_data_oracle(org_id, srxm_list_that_month_min, srxm_qr, that_month_min, that_month_max):
    # 请洗之前 先根据主键REPORT_BEGIN_DATE判断在SCADA_REPORT_XN_WEEK有没有 有的话 先删除 再清洗入库
    deal_with_data_for_oracle_srxmonth_del_all_with_where(org_id, that_month_min)
    # 继续清洗数据的逻辑
    # 对 srxm_list_that_month_min 进行循环遍历 拿出其 SCADA_REPORT_XN_MID 的 表计编号
    for srxm in srxm_list_that_month_min:
        flmeter_no = srxm.FLMETER_NO
        # print(flmeter_no)
        # 通过 flmeter_no 和 org_id 和 that_week_max 再去 SCADA_REPORT_XN_MID 拿数据 拿七天之后 周一的数据
        # 首先判空 空的话 跳出本次循环
        if flmeter_no == None:
            continue
        # 不空的话 开始查询月最后一天期末mid数据数据
        srxm_list_that_week_max, srxm_qr = deal_with_data_for_oracle_srxm_select_where_with_org_id_and_flmeter_no(org_id, flmeter_no, that_month_max)
        # 定义一些公关变量吧
        # 周二期初数据 mrm_list_that_week_min
        begin_std_sum = srxm.STD_SUM
        begin_price = srxm.PRICE
        ...
        if len(srxm_list_that_week_max) >= 1:
            # 查询出数据 给 end_* 赋值
            print('...机构号:', org_id, '...Oracle...srxm_list...len...查询ScadaReportXNMid总条数: ' + str(len(srxm_list_that_week_max)), ' 时间: ', that_month_max, '机构号: ', org_id, '表号: ', flmeter_no, '查询出对应月末结束日期数据一条 去正常处理')
            # 给 周一期末数据 赋值
            end_srxm = srxm_list_that_week_max[0]
            end_std_sum = end_srxm.STD_SUM
            end_price = end_srxm.PRICE
            ...
            # 都置成0 入库 continue掉 即可
            # 入库 continue 掉即可 写一个就行
            tmp = ScadaReportXNMonth(SRXM_ORG_ID=org_id, FLMETER_NO=flmeter_no, COMM_NO=srxm.COMM_NO,
                                    CUSTOMER_NO=customer_no,
                                    REPORT_BEGIN_DATE=that_month_min.date(),
                                    BEGIN_STD_SUM=begin_std_sum,
                                    BEGIN_PRICE=begin_price,
                                    BEGIN_THIS_CYCLE_SUM=begin_this_cycle_sum,
                                    REPORT_END_DATE=that_month_max.date(),
                                    END_STD_SUM=("0" if (end_std_sum is None) else str(end_std_sum)),
                                    END_PRICE=("0" if (end_price is None) else str(end_price)),
                                    END_THIS_CYCLE_SUM=("0" if (end_this_cycle_sum is None) else str(end_this_cycle_sum)),
                                    USE_VOLUME_STD="0",
                                    ...
                                    GRADE_PRICE3=("0" if (gp3 is None) else str(gp3)))
            commit()  # 提交事务
            print('...机构号:', org_id, '...Oracle...srxm_list...len...查询ScadaReportXNMid总条数: ' + str(len(srxm_list_that_week_max)), ' 时间: ', that_month_max, '机构号: ', org_id, '表号: ', flmeter_no, '没有查询出对应月末结束日期数据一条 均按照为0处理 入库 同时continue掉 进行下一个循环')
            continue
        # 初始化数据完毕 开始数据清洗 数据收集 最后入库前的处理
        # 截取最后一位为价格判断符price_flag
        price_flag = None
        ...
        tmp = ScadaReportXNMonth(SRXM_ORG_ID=org_id, FLMETER_NO=flmeter_no, COMM_NO=srxm.COMM_NO,
                                ...
                                GRADE_PRICE3=("0" if (gp3 is None) else str(gp3)))
        commit()  # 提交事务
        print('Done...机构号:', org_id, '...Oracle...datas_from_mid_to_month_wash_data_oracle...flmeter_no表计编号: ' + flmeter_no, '清洗成功 入库ScadaReportXNMonth成功', '对应REPORT_BEGIN_DATE为:', that_month_min)
    pass


# deal_with_data_for_oracle_srxmonth_del_all_with_where
# SCADA_REPORT_XN_MONTH
# 对应一个【报表开始日期，月初】 通过 REPORT_BEGIN_DATE 进行条件判断
# 批量删除 - 带条件 直接删
# org_id 机构号
# that_month_min 月初日期【其就是 REPORT_BEGIN_DATE】传入即可
# 返回 删除行数
@db_session
def deal_with_data_for_oracle_srxmonth_del_all_with_where(org_id, that_month_min):
    this_date = that_month_min.date()
    rowcount = delete(srxmonth for srxmonth in ScadaReportXNMonth if srxmonth.REPORT_BEGIN_DATE == this_date and srxmonth.SRXM_ORG_ID == org_id)
    print('...清洗之前...先删除...机构号:', org_id, '...Oracle...deal_with_data_for_oracle_srxmonth_del_all_with_where...删除ScadaReportXNMonth总条数: ' + str(rowcount) + ' 对应REPORT_BEGIN_DATE月初开始周期时间为: ', that_month_min)
    return rowcount
