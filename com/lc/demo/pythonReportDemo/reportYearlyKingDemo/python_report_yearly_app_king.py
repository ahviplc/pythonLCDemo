...
python_report_yearly_app_king.py 加强版本 封装了年报表对象类以及将取自动递增流水方法提取到工具db_utils文件中,集成监听所有的print到log日志的封装类
...
# @param  日报表对象report_yearly_model-主键【sry_org_id 机构号,sry_id 记录ID 】其他字段
# @return 处理结果 True成功 False失败
...
def data_processing(data_for_processing, org_id, **kwargs):
    ...
    for fno in flmeter_no_set:
        ...
        rym.use_volume_work = str(int(max_work_sum) - int(min_work_sum))
        # 周期内标况使用量（周期内期末数 - 期初数）
        ...
        rym.use_volume_std = str(int(max_std_sum) - int(min_std_sum))  # 周期内标况使用量（周期内期末数-期初数）
...
if __name__ == '__main__':
    ...
    # sys.stdout = PrintLogger('python_report_yearly_app_king.py.log')  # 监听所有的print到log日志 封装类 如不需要打印所有输出print的log日志，隐掉这段即可
    ...
    if len(return_data) > 0:
        ...
        is_ok = data_processing(return_data, params_data[0]['orgid'], query_datetime=params_data[0]['minTime'])  # 数据处理函数，处理年报表 , 年报表数据计算，写入数据库操作
        if is_ok:
