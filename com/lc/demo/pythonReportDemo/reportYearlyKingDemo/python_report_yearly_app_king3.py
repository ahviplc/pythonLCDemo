...
python_report_yearly_app_king3.py 加强版本3
再次优化,此次优化，不直接从SCADA_FLMETER_DATA计算算出了,从月报表SCADA_REPORT_MONTHLY里计算算出，减少计算量
封装了年报表对象类以及将取自动递增流水方法提取到工具db_utils文件中,集成监听所有的print到log日志的封装类
年报表-计算写入数据库oracle的报表脚本
版本说明:1：跑所有机构的年报表；2:逻辑变更-【周期内工况使用量（本期期末数-上期期末数）】【周期内标况使用量（本期期末数-上期期末数）】 3:整体脚本代码结构变更
Version: 1.0
Author: LC
DateTime: 2019年8月16日11:21:21
UpdateTime: 2019年8月16日11:21:27
...
# 从oracle数据库SCADA_REPORT_MONTHLY读取所有符合条件的数据 for 年报表
# 带参数查询
# @param  srm_org_id 要查询机构号
# @param  years 要查询的年 0代表当前当年 -n代表前n年 +n代表后n年
# @return 处理结果 True成功 False失败
def select_srm_by_where_for_yearly(srm_org_id, years):
    sql = "select * from SCADA_REPORT_MONTHLY where SRM_ORG_ID = :srm_org_id and YEAR = :years "
    year_first_min = to_get_year_first_last_day_datetime_max_min_time(years, "first", "min", False)  # 方法:获取间隔n年的第一天的最小时间和最后一天的最大时间
    year_last_max = to_get_year_first_last_day_datetime_max_min_time(years, "last", "max", False)
    # 注意 在此逻辑中不需要【间隔n年的第一天的最小时间和最后一天的最大时间】 我们只需要其 年
    ok_year = year_first_min.year
    data = [{"srm_org_id": srm_org_id, "years": ok_year}]
    fc = db.select_by_where_many_params_dict(sql, data)
    print(ok_year, "总共SCADA_REPORT_MONTHLY数据:", len(fc))
    # for row in fc:
    #     print(row)
    return fc, data
...
# 周期内相加值方法
# @param data_list 计算的字典列表 key 对应的键
# @return 处理之后的周期内相加值-返回四舍五入-再处理成str类型返回
def get_plus_value(data_list, key):
    count_nums = 0
    total_size = len(data_list)
    for x in data_list:
        if x[key] is not None:
...
# 周期内求最大值最小值方法
# @param data_list 计算的字典列表 key 对应的键 types 类型 max 求最大值 min 求最小值 这里的list已经通过month排过序了
# @return 处理之后的周期内最大值最小值-返回四舍五入-再处理成str类型返回
def get_max_min_value(data_list, key, types):
    count_nums = 0
    total_size = len(data_list)
    max_value = 0
    min_value = 0
    sorted_data_list = sorted(data_list, key=operator.itemgetter(key), reverse=False)
    min_value = sorted_data_list[0][key]
    max_value = sorted_data_list[total_size-1][key]  # 默认升序，列表最后一个元素，值最大
    ok_value = 0
    ok_time = None
    if types == "max":
        ok_value = float(max_value)
        ok_time = sorted_data_list[total_size-1]['MAX_STD_FLOW_TIME']
    if types == "min":
        ok_value = float(min_value)
        ok_time = sorted_data_list[0]['MIN_STD_FLOW_TIME']
    return str(round(ok_value, 2)), ok_time  # 返回四舍五入
...
def data_processing(data_for_processing, last_data_for_processing, org_id, **kwargs):
    ...
    last_rm_repeat_sfd_data_list = []  # 用于临时存放已删除重复的字典数据 上一年的 上一次的
    ...
    for fno in flmeter_no_set:
        ...
        if len(last_rm_repeat_sfd_data_list) > 0:  # （本期期末数-上期期末数）
            rym.use_volume_work = str(round(float(max_work_sum) - float(last_sorted_rm_repeat_sfd_data_list[len(last_rm_repeat_sfd_data_list) - 1]['WORK_SUM']), 2))
        else:  # （本周期内期末数-本周期内期初数）
            rym.use_volume_work = str(round(float(max_work_sum) - float(min_work_sum), 2))
        ...
        if len(last_rm_repeat_sfd_data_list) > 0:  # （本期期末数-上期期末数）
            ...
            rym.use_volume_std = str(round(float(max_std_sum) - float(last_sorted_rm_repeat_sfd_data_list[len(last_rm_repeat_sfd_data_list) - 1]['SUM_TOTAL']), 2))
        else:  # 周期内标况使用量（周期内期末数-期初数）
            rym.use_volume_std = str(round(float(max_std_sum) - float(min_std_sum), 2))
...
# 数据处理-主逻辑处理-主要函数方法 data_processing2 此逻辑大修改,从SCADA_REPORT_MONTHLY中取值 计算
# @param data_for_processing 要处理的原数据
# @param last_data_for_processing 要处理的原数据-上一次的
# @param org_id 机构号
# @param 字典传参 query_datetime 查询操作的日期
# @return 处理结果 True成功 False失败
def data_processing2(data_for_processing, last_data_for_processing, org_id, **kwargs):
    rm_repeat_srm_data_list = []  # 用于临时存放已删除重复的字典数据
    last_rm_repeat_srm_data_list = []  # 用于临时存放已删除重复的字典数据
    ...
    for fno in flmeter_no_set:
        ...
        for xx in data_for_processing:
            if xx['FLMETER_NO'] == fno:
                rm_repeat_srm_data_list.append(xx)
            # print(rm_repeat_srm_data_list)
        # print(len(rm_repeat_srm_data_list))

        # last数据处理
        for xxx in last_data_for_processing:
            if xxx['FLMETER_NO'] == fno:
                last_rm_repeat_srm_data_list.append(xxx)
            # print(last_rm_repeat_srm_data_list)
        # print(len(last_rm_repeat_srm_data_list))

        # 废弃 将std_flow，work_flow，temperature,pressure 全部转为float,再继续操作,为后面数字排序做准备
        # 废弃 查询当年的上一年数据中 将std_flow，work_flow，temperature,pressure 全部转为float,再继续操作,为后面数字排序做准备
        # 备注: 此rm_repeat_srm_data_list中的所有字段数据 无需再次加工 肯定不会有空的 即使原来从拍那个sfd中 拿来的数据是空 我也在跑月脚本的时候 给它设置了默认值

        # print(rm_repeat_srm_data_list)
        print("此查询区间,当前编号下总共SCADA_REPORT_MONTHLY数据:", len(rm_repeat_srm_data_list))
        print("此查询区间,上一年当前编号下总共SCADA_REPORT_MONTHLY数据:", len(last_rm_repeat_srm_data_list))

        # 此表计数据字典列表 排序 按照采集时间MONTH排序 默认升序 如果要降序排序,可以指定reverse=True
        sorted_rm_repeat_srm_data_list = sorted(rm_repeat_srm_data_list, key=operator.itemgetter('MONTH'), reverse=False)

        # 上一年SCADA_REPORT_MONTHLY数据 排序 按照采集时间MONTH排序 默认升序 如果要降序排序,可以指定reverse=True
        last_sorted_rm_repeat_srm_data_list = []
        if len(last_rm_repeat_srm_data_list) > 0:  # 如果大于0 进行以下操作
            last_sorted_rm_repeat_srm_data_list = sorted(last_rm_repeat_srm_data_list, key=operator.itemgetter('MONTH'), reverse=False)
        ...
        # 机构号
        rym.sry_org_id = sorted_rm_repeat_srm_data_list[0]['SRM_ORG_ID']

        # 记录id srd_id 移到line385

        # RTU编号
        rym.rtu_no = sorted_rm_repeat_srm_data_list[0]['RTU_NO']
        # 流量计编号
        rym.flmeter_no = sorted_rm_repeat_srm_data_list[0]['FLMETER_NO']
        # 客户编号
        rym.customer_no = sorted_rm_repeat_srm_data_list[0]['CUSTOMER_NO']
        ...
        # 将查询时间的年月日 分别赋值到对应字段
        # 处理年
        rym.year = str(kwargs['query_datetime'])  # query_datetime就是当前处理的年
        # 处理月 不处理了 togo
        # print(len(str(rym.month)))
        # 如果月份小于10 补零 让9变为09月
        # if len(str(kwargs['query_datetime'].month)) < 2:
        ...
        # 期末数的都取月中最大月的值    周期内的都相加即可

        # 标况总量（期末数）
        rym.std_sum = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['STD_SUM']
        # 工况总量（期末数）
        rym.work_sum = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['WORK_SUM']

        # 标况流量（周期内最大值）最大标况流量时间 STD_FLOW
        rym.max_std_flow, rym.max_std_flow_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MAX_STD_FLOW", "max")  # 求最大值
        # 标况流量（周期内最小值）最小标况流量时间
        rym.min_std_flow, rym.min_std_flow_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MIN_STD_FLOW", "min")  # 求最小值
        # 标况流量（周期内平均值）- rym.std_flow
        rym.avg_std_flow = get_average_period(sorted_rm_repeat_srm_data_list, "AVG_STD_FLOW")  # 使用周期内平均值计算方法 计算平均值

        # 工况流量（周期内最大值）最大工况流量时间 WORK_FLOW
        rym.max_work_flow, rym.max_work_flow_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MAX_WORK_FLOW", "max")  # 求最大值
        # 工况流量（周期内最小值）最小工况流量时间
        rym.min_work_flow, rym.min_work_flow_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MIN_WORK_FLOW", "min")  # 求最小值
        # 工况流量（周期内平均值）-rym.work_flow
        rym.avg_work_flow = get_average_period(sorted_rm_repeat_srm_data_list, "AVG_WORK_FLOW")

        # 温度（周期内最大值）最高温度时间 TEMPERATURE
        rym.max_temperature, rym.max_temp_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MAX_TEMPERATURE", "max")  # 求最大值
        # 温度（周期内最小值）最低温度时间
        rym.min_temperature, rym.min_temp_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MIN_TEMPERATURE", "min")  # 求最小值
        # 温度（周期内平均值）-rym.temperature
        rym.avg_temperature = get_average_period(sorted_rm_repeat_srm_data_list, "AVG_TEMPERATURE")

        # 压力（周期内最高值）最高压力时间 PRESSURE
        rym.max_press, rym.max_press_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MAX_PRESS", "max")  # 求最大值
        # 压力（周期内最低值）最低压力时间
        rym.min_press, rym.min_press_time = get_max_min_value(sorted_rm_repeat_srm_data_list, "MIN_PRESS", "min")  # 求最小值
        # 压力（周期内平均值）-rym.pressure
        rym.avg_press = get_average_period(sorted_rm_repeat_srm_data_list, "AVG_PRESS")

        # 单价（期末数）
        rym.price = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['PRICE']

        # 周期内工况使用量（周期内期末数-期初数） 相加
        rym.use_volume_work = get_plus_value(sorted_rm_repeat_srm_data_list, "USE_VOLUME_WORK")  # 求和

        # 周期内标况使用量（周期内期末数 - 期初数） 相加
        rym.use_volume_std = get_plus_value(sorted_rm_repeat_srm_data_list, "USE_VOLUME_STD")  # 求和

        # 周期内使用额（单价（期末数）* 周期内标况使用量）结果四舍五入
        rym.use_money = get_plus_value(sorted_rm_repeat_srm_data_list, "USE_MONEY")  # 求和

        # 总累积使用量（期末数）
        rym.sum_total_volume = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['SUM_TOTAL_VOLUME']
        if rym.sum_total_volume is None:
            rym.sum_total_volume = str(0)
            print(rym.flmeter_no, "☆ sum_total_volume is None 置为0")

        # 累购气量（期末数）
        rym.total_buy_volume = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['TOTAL_BUY_VOLUME']
        # 累购金额（期末数）
        rym.total_buy_money = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['TOTAL_BUY_MONEY']
        # 剩余金额（期末数）
        rym.remain_money = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['REMAIN_MONEY']
        # 总累计使用金额（期末累购金额-期末剩余金额）
        if rym.total_buy_money is None:  # total_buy_money为None的话 置为0查询计算
            rym.total_buy_money = str(0)
        if rym.remain_money is None:  # remain_money为None的话 置为0查询计算
        ...
        # 剩余数量（期末数）
        rym.remain_volume = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['REMAIN_VOLUME']
        # 流量计(表)状态（期末数）
        rym.fm_state = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['FM_STATE']
        # 表状态解析（按位解析）（期末数）
        rym.fm_state_msg = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['FM_STATE_MSG']
        # RTU状态（期末数）
        rym.rtu_state = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['RTU_STATE']
        # RTU状态解析（按字节解析）（期末数）
        rym.rtu_state_msg = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['RTU_STATE_MSG']
        # 阀门控制器状态（期末数）
        rym.valve_state = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['VALVE_STATE']
        # 阀门控制器状态解析（期末数）
        rym.valve_state_msg = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['VALVE_STATE_MSG']
        # 供电电压（期末数）
        rym.power_voltage = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['POWER_VOLTAGE']

        # 电池电压（期末数）
        rym.battery_voltage = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['BATTERY_VOLTAGE']
        # 电池电量（期末数）
        rym.battery_level = sorted_rm_repeat_srm_data_list[len(sorted_rm_repeat_srm_data_list) - 1]['BATTERY_LEVEL']
        # 入口压力（周期内平均值）
        rym.press_in = get_average_period(sorted_rm_repeat_srm_data_list, "PRESS_IN")
        # 出口压力（周期内平均值）
        rym.press_out = get_average_period(sorted_rm_repeat_srm_data_list, "PRESS_OUT")
        # 入口温度（周期内平均值）
        rym.temp_in = get_average_period(sorted_rm_repeat_srm_data_list, "TEMP_IN")

        # 出口温度（周期内平均值）
        rym.temp_out = get_average_period(sorted_rm_repeat_srm_data_list, "TEMP_OUT")
        # 信号强度（平均值）
        rym.rssi = get_average_period(sorted_rm_repeat_srm_data_list, "RSSI")
        # 删除标识符 1正常，9不正常已删除 默认置为1
        rym.sry_status = "1"

        # print(sorted_rm_repeat_srm_data_list)
        # print(len(sorted_rm_repeat_srm_data_list), sorted_rm_repeat_srm_data_list[0]['FLMETER_NO'], max_std_sum,min_std_sum, ok_std_sum)
        # print('----------------------------------------------------------------------------------------')
        ...
        # 处理数据完毕 清除临时使用数据
        flmeter_no_set_copy.remove(fno)
        rm_repeat_srm_data_list.clear()
        last_rm_repeat_srm_data_list.clear()
    pass
    return True
...
def main(db, org_id, years):
    ...
    return_data, params_data = select_sfd_by_where_for_yearly(org_id, years)  # @param org_id 要查询机构号 @param years 0代表当前年 +n代表前n年 -n代表后n年 默认为-1 跑上一年的数据
...
# main2方法 逻辑大修改 从月报表里取值
# @param db 数据库实例
# @param org_id 机构号
# @param years 天数
# @return main方法运行处理结果 执行完毕即可
def main2(db, org_id, years):
    ...
    # 年相关
    # 从SCADA_REPORT_MONTHLY中查询此years年的1月到最大月的数据
    # this_year_scada_report_monthly_list, this_year_params_data
    return_data, params_data = select_srm_by_where_for_yearly(org_id, years)  # @param org_id 要查询机构号 @param years 0代表当前年 +n代表后n年 -n代表前n年 默认为-1 跑上一年的数据

    # 从SCADA_REPORT_MONTHLY中查询此years-1年1月到最大月的数据
    # last_year_scada_report_monthly_list, last_year_params_data
    last_return_data, last_params_data = select_srm_by_where_for_yearly(org_id, years-1)  # @param org_id 要查询机构号 @param years 0代表当前年 +n代表后n年 -n代表前n年 默认为-1 跑上一年的数据
    ...
    # 接下来开始处理查询出数据
    if len(return_data) > 0:
        print(params_data[0]['srm_org_id'], params_data[0]['years'], "开始进行计算年报表数据处理")
        is_ok = data_processing2(return_data, last_return_data, params_data[0]['srm_org_id'], query_datetime=params_data[0]['years'])  # 数据处理函数，处理年报表 , 年报表数据计算，写入数据库操作
        if is_ok:
            print(params_data[0]['srm_org_id'], "data_processing is ok")
            print('----------------------------------------------------------------------------------------')
        pass
    else:
        print(params_data[0]['srm_org_id'], params_data[0]['years'], "期间无SCADA_REPORT_MONTHLY数据，请等待重新计算年报表")
        print('----------------------------------------------------------------------------------------')
    pass
...
if __name__ == '__main__':
    ...
    # sys.stdout = PrintLogger('python_report_yearly_app_king3.py.log')  # 监听所有的print到log日志 封装类 如不需要打印所有输出print的log日志，隐掉这段即可
    ...
    for x in org_list:
        print("此机构:", x['ORG_ID'])
        main2(db, x['ORG_ID'], 0)  # 传入的机构,设置要查询哪年！运行main方法，将db带过去，机构id， -1跑上年的数据！0跑今年的数据！用于下面的操作！
