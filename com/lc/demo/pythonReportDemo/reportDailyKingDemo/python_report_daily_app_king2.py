...
python_report_daily_app_king2.py 加强版本2 封装了日报表对象类以及将取自动递增流水方法提取到工具db_utils文件中,集成监听所有的print到log日志的封装类
...
def data_processing(data_for_processing,last_data_for_processing,org_id, **kwargs):
    ...
    for fno in flmeter_no_set:
        ...
        if len(last_rm_repeat_sfd_data_list) > 0:  # （本期期末数-上期期末数）
            rdm.use_volume_std = str(float(max_std_sum) - float(last_sorted_rm_repeat_sfd_data_list[len(last_rm_repeat_sfd_data_list) - 1]['STD_SUM']))  # 周期内标况使用量（周期内期末数-期初数）
...
if __name__ == '__main__':
    ...
    # sys.stdout = PrintLogger('python_report_daily_app_king2.py.log')  # 监听所有的print到log日志 封装类 如不需要打印所有输出print的log日志，隐掉这段即可
