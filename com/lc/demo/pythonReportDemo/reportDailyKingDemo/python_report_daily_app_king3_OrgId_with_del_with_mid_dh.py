...
python_report_daily_app_king3_OrgId_with_del_with_mid_dh.py 跑之前先删除所有的相关数据 带机构号版本 加强版本2 封装了日报表对象类以及将取自动递增流水方法提取到工具db_utils文件中,集成监听所有的print到log日志的封装类
...
版本说明:1：跑所有机构的日报表；2:逻辑变更-【周期内工况使用量（本期期末数-上期期末数）】【周期内标况使用量（本期期末数-上期期末数）】 3:整体脚本代码结构变更
        ...
        5: 现在查询 0089 ['2022-01-09 00:00:00', '2022-01-10 00:00:00'] 抄表数据 来算1月9号的日报表了
Version: 1.0
Author: LC
DateTime: 2019年3月7日14:16:04
UpdateTime: 2022年1月10日09:52:26
...
# @param  days 0代表今天 +n代表n天后 -n代表n天前
# @return 处理结果 True成功 False失败
def select_sfd_by_where_with_mid_dh_2(org_id, days):
...
if __name__ == '__main__':
    ...
    # sys.stdout = PrintLogger('python_report_daily_app_king3_OrgId_with_del_with_mid_dh.py.log')  # 监听所有的print到log日志 封装类 如不需要打印所有输出print的log日志，隐掉这段即可
