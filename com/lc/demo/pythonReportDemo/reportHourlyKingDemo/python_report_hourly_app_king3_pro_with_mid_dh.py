...
"""
目前使用的是这个
python_report_hourly_app_king3_pro_with_mid_dh.py 加强版本3_pro 优化sql 再次优化小时报表 封装了小时报表对象类以及将取自动递增流水方法提取到工具db_utils文件中,集成监听所有的print到log日志的封装类
...
# 从oracle数据库SCADA_FLMETER_DATA读取所有符合条件的数据 select_sfd_by_where_for_hourly_ver_2_with_mid_dh 使用中间表【SCADA_FLMETER_DATA_MID_DH】
# 带参数查询
# @param  org_id 要查询机构号
# @param days 天数 代表几天，可以正值(n天后)，可以负值(n天前),0代表今天
# @param  hours 0代表当前小时 +n代表n小时后 -n代表n小时前
# @return 处理结果 True成功 False失败
def select_sfd_by_where_for_hourly_ver_2_with_mid_dh(org_id, days, hours):
...
# 从oracle数据库SCADA_FLMETER_DATA读取所有符合条件的数据 select_sfd_by_where_for_hourly_ver_2_pro_with_mid_dh 使用中间表【SCADA_FLMETER_DATA_MID_DH】
# 带参数查询
# @param  org_id 要查询机构号
# @param days 天数 代表几天，可以正值(n天后)，可以负值(n天前),0代表今天
# @param  hours 0代表当前小时 +n代表n小时后 -n代表n小时前
# @return 处理结果 True成功 False失败
def select_sfd_by_where_for_hourly_ver_2_pro_with_mid_dh(org_id, days, hours):
    sql = "select * from SCADA_FLMETER_DATA_MID_DH where SFD_ORG_ID= :orgid and INSTANT_TIME between :minTime AND :maxTime "
    yesterday_min = to_get_hour_first_last_minutes_datetime_max_min_time_ver_2(days, hours - 1, "max", False)
    yesterday_max = to_get_hour_first_last_minutes_datetime_max_min_time_ver_2(days, hours, "max", False)
    data = [{"orgid": org_id, "minTime": yesterday_min, "maxTime": yesterday_max}]
...
# 从oracle数据库SCADA_FLMETER_DATA读取所有符合条件的数据  凌晨1点数据计算专属方法 select_sfd_by_where_for_hourly2_with_mid_dh 使用中间表【SCADA_FLMETER_DATA_MID_DH】
# 带参数查询
# @param  org_id 要查询机构号
# @param days 天数 代表几天，可以正值(n天后)，可以负值(n天前),0代表今天
# @param  hours 0代表当前小时 +n代表n小时后 -n代表n小时前
# @return 处理结果 True成功 False失败
def select_sfd_by_where_for_hourly2_with_mid_dh(org_id, days, hours):
    sql = "select * from SCADA_FLMETER_DATA_MID_DH where SFD_ORG_ID= :orgid and INSTANT_TIME between :minTime AND :maxTime "
    yesterday_min_max = to_get_hour_first_last_minutes_datetime_max_min_time2(days, hours, "min", False)
    yesterday_max_min = to_get_hour_first_last_minutes_datetime_max_min_time2(days, hours+23, "min2", False)
    data = [{"orgid": org_id, "minTime": yesterday_max_min, "maxTime": yesterday_min_max}]
...
def main(db, org_id, days, hours):
    ...
    if hours == -23: # 凌晨1点的数据
        ...
        # return_data, params_data = select_sfd_by_where_for_hourly_ver_2_pro(org_id, days, hours)  # 这样写 和下行也是一样的
        ...
        return_data, params_data = select_sfd_by_where_for_hourly_ver_2_with_mid_dh(org_id, days, hours)  # 使用中间表
        ...
        last_return_data, last_params_data = select_sfd_by_where_for_hourly2_with_mid_dh(org_id, days, hours - 1)  # 使用中间表
        ...
        return_data, params_data = select_sfd_by_where_for_hourly_ver_2_pro_with_mid_dh(org_id, days, hours)  # 使用中间表
        ...
        last_return_data, last_params_data = select_sfd_by_where_for_hourly_ver_2_pro_with_mid_dh(org_id, days, hours - 1)  # 使用中间表
...
if __name__ == '__main__':
    ...
    # sys.stdout = PrintLogger('python_report_hourly_app_king3_pro_with_mid_dh.py.log')  # 监听所有的print到log日志 封装类 如不需要打印所有输出print的log日志，隐掉这段即可
    ...
    for x in org_list:
        print("此机构:", x['ORG_ID'])
        if x['ORG_ID'] == '0098':
            print("☆0098△ 此机构:", x['ORG_ID'], " 0098机构不跑小时报表 阻断一下(for循环-continue了) 其他继续")
            print("----------------------------------------------------------------------------------------")
            continue
        if x['ORG_ID'] == '0089':
            print("☆0089△ 此机构:", x['ORG_ID'], " 0089机构不跑小时报表 阻断一下(for循环-continue了) 其他继续")
            print("----------------------------------------------------------------------------------------")
            continue
        if x['ORG_ID'] == '0080':
            print("☆0080△ 此机构:", x['ORG_ID'], " 0080机构不跑小时报表 阻断一下(for循环-continue了) 其他继续")
            print("----------------------------------------------------------------------------------------")
            continue
        if x['ORG_ID'] == '0048':
            print("☆0048△ 此机构:", x['ORG_ID'], " 0048机构不跑小时报表 阻断一下(for循环-continue了) 其他继续")
            print("----------------------------------------------------------------------------------------")
            continue
        if x['ORG_ID'] == '0014':
            print("☆0014△ 此机构:", x['ORG_ID'], " 0014机构不跑小时报表 阻断一下(for循环-continue了) 其他继续")
            print("----------------------------------------------------------------------------------------")
            continue
