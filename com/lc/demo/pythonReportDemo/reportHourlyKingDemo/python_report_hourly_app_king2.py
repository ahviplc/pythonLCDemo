...
python_report_hourly_app_king2.py 加强版本2 封装了小时报表对象类以及将取自动递增流水方法提取到工具db_utils文件中,集成监听所有的print到log日志的封装类
...
def data_processing(data_for_processing, last_data_for_processing, org_id, **kwargs):
    ...
    for fno in flmeter_no_set:
        ...
        if len(last_rm_repeat_sfd_data_list) > 0:  # （本期期末数-上期期末数）
            ...
            rdm.use_volume_work = str(float(max_work_sum) - float(last_max_work_sum))
...
def main(db, org_id, days, hours):
    ...
    if hours == -23: # 凌晨1点的数据
        # 设置查询的机构,要查询哪一小时
        return_data, params_data = select_sfd_by_where_for_hourly(org_id, days, hours)  # @param org_id 要查询机构号 @param days 代表几天，可以正值(n天后)，可以负值(n天前),0代表今天; hours 0代表小时 +n代表n小时后 -n代表n小时前 默认为-1 跑1小时前的数据
        ...
        # 查询当天的上一天数据
        print("下面是查询当前小时的上一小时数据-总共抄表数据")
        last_return_data, last_params_data = select_sfd_by_where_for_hourly(org_id, days, hours - 1)
    ...
    if len(return_data) > 0:
        ...
        is_ok = data_processing(return_data, last_return_data, params_data[0]['orgid'],query_datetime=params_data[0]['maxTime'])  # 数据处理函数，处理小时报表 , 小时报表数据计算，写入数据库操作
        if is_ok:
...
if __name__ == '__main__':
    ...
    # sys.stdout = PrintLogger('python_report_hourly_app_king2.py.log')  # 监听所有的print到log日志 封装类 如不需要打印所有输出print的log日志，隐掉这段即可
