...
python_report_hourly_app_king4_threading.py 加强版本4 多线程版本 再次优化小时报表 封装了小时报表对象类以及将取自动递增流水方法提取到工具db_utils文件中,集成监听所有的print到log日志的封装类
...
# for main函数
def for_main(name, org_list):
    which_day = -1
    print('---开始--- ', name, ' 时间', time.ctime())
    for x in org_list:
        print("此机构:", x['ORG_ID'], name)
        if x['ORG_ID'] == '0048':
            print("☆0048△ 此机构:", x['ORG_ID'], name," 0048机构不跑小时报表 阻断一下(for循环-continue了) 其他继续")
            print("----------------------------------------------------------------------------------------")
            continue
        for hours_temp in range(-23, 1, +1):
            main(db, x['ORG_ID'], which_day, hours_temp)  # 传入的机构,设置要查询哪一天哪一小时！运行main方法，将db带过去，机构id，-1,-0 => -1跑昨天的数据！-0代表昨天0-24点数据，用于下面的操作！
