...
"""
tetris_with_notes.py
俄罗斯方块 小游戏
Version: 1.0
Author: LC
DateTime: 2019年7月9日14:30:03
UpdateTime:
...
def block_move_down():
    ...
    pygame.display.set_caption('Tetris,Score:' + str(score[0]) + ' LC')  # 直接把得分打在窗口标题上
    ...
    for row, column in select_block:
        if background[y_drop + row][x_move + column]:
            # print(ground[row][column])  # 下行bug fix 如果background的每一行都有方块 就说明方块已经叠加到窗口顶部
            print(background[row][column])  # 如果background的每一行都有方块 就说明方块已经叠加到窗口顶部
